...
// 目標：理解介面擴充多重介面的寫法。
type Tcurrency = "TWD"|"USD"
export interface Price { price:number, currency:Tcurrency}
...
export const fetchPlants = async ():Promise<PlantDTO[]> /* TODO */ => {
  const res:AxiosResponse<PlantDTO[]> = await axios.get('https://fakestoreapi.com/products');
  return res.data
}
...
export function updatePlant(input:Partial<PlantBase>):Required<PlantBase> /* TODO */ {
...
export type TInventory= Record<Inventory, number>
export const inventory:TInventory /* TODO */ = {
  "PLANT-1001": 42,
  "PLANT-2001": 8,
};
...
type CreateProduct = Omit<TProduct, "id">
...
*/
type OptionalColumn = Omit<TProduct, "id" | "title">
type RequiredColumn = Pick<TProduct, "id" | "title">

type UpdateProduct = Partial<OptionalColumn>& RequiredColumn
/*
...
function submitProduct (type:Action.create, product:CreateProduct):string
function submitProduct (type:Action.update, product:UpdateProduct):string
function submitProduct (type:Action, product:CreateProduct|UpdateProduct):string{
  switch (type){
    case Action.create:
